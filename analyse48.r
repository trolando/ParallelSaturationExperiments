...
solvedSummary <- times %>% group_by(Method, Order, Workers) %>% summarize(Count = n_distinct(Model)) %>% spread(Workers, Count) %>% left_join(times %>% group_by(Method, Order) %>% summarize(Cumulative = n_distinct(Model))) %>% left_join(bind_rows(times_s, timeouts_s) %>% group_by(Order) %>% summarize(Max = n_distinct(Model)))
...
LDDSAT1_MDDSAT1plot <-
    ...
    scale_y_log10(name="Time with Sylvan LDDs (sec)", limits=c(0.0001,1200)) + 
    theme_bw()
